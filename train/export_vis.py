...
def evaluate_sentiment_once(sess, writer, 
  ...
  try:
    while True:
      ...
      vid = ids[0]
      vid, n_frames = ids[0], prediction_vals['n_frames'][0]
      ...
      for i in xrange(n_frames):
        frame = { 'sentiments': map(lambda x: round(x, 3), prediction_vals['sentiments'][0, i].tolist()) }
        if 'topics' in prediction_vals:
          frame['topics'] = map(lambda x: round(x, 3), prediction_vals['topics'][0, i].tolist())
        frames.append(frame)
      ...
      filename = os.path.join(FLAGS.vis_dir, '{}.json'.format(vid))
      with open(filename, 'w') as fp:
        fp.write(json.dumps(results[vid]))
...
def main(_):
  ...
  with g.as_default():
    ...
    predictions = model.build_step_inference(examples)
